...
export default function MapPage() {
  const [mapData, setMapData] = useState({ doctors: [], cruelty: [] })
      ...
      {loading ? <LoadingSpinner /> : (
        <>
          <div className="rounded-2xl border border-zinc-800 bg-zinc-900/60 p-2 backdrop-blur">
            <MapView doctors={mapData.doctors} cruelty={mapData.cruelty} />
          </div>
          ...
          {mapData.doctors.length === 0 && mapData.cruelty.length === 0 && (
