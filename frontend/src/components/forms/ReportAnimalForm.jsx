...
export default function ReportAnimalForm({ onSuccess }) {
  ...
  const [form, setForm] = useState({
    animal_type: '', location: '', description: '', contact_name: user?.name || '', contact_phone: user?.phone || ''
  })
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      onSuccess?.(res.data)
      setForm({ animal_type: '', location: '', description: '', contact_name: user?.name || '', contact_phone: user?.phone || '' })
    } catch (err) {
